import FooterArrow from "@/components/FooterArrow.jsx";

function Footer() {
  return (
    <footer>
      <div className="pl-[8.5%] pr-[7%] py-[8.5%]">
        <header className="mb-7 lg:mb-14">
          <div>
            <p className="text-[2.4rem] lg:text-6xl font-medium">Ableton</p>
          </div>
        </header>
        <div className="text-[0.94rem] xl:text-[1.2rem] lg:flex flex-wrap">
          <div className="order-2 basis-2/5">
            <form action="https://www.ableton.com/en/subscribe-to-newsletter/">
              <label htmlFor="email">
                <p className="font-semibold">Sign up to our newsletter</p>
                <p>
                  Enter your email address to stay up to date with the latest
                  offers, tutorials, downloads, surveys and more.
                </p>
              </label>
              <div className="flex mt-5 mb-8 flex-wrap">
                <input
                  type="text"
                  id="email"
                  placeholder="Email Address"
                  className="bg-[#eeeeee] py-1.5 pl-3 grow"
                />
                <button
                  type="submit"
                  className="bg-[#0000FF] text-white font-semibold py-1.5 px-6"
                >
                  Sign up
                </button>
              </div>
            </form>
          </div>
          <div className="order-1 basis-3/5 lg:grid grid-cols-2 grid-rows-2">
            <div>
              <ul>
                <li>
                  <a
                    href="https://www.ableton.com/en/account/add_license/"
                    className="inline-block"
                  >
                    <div className="flex items-center">
                      <p>Register Live or Push</p>
                      <FooterArrow />
                    </div>
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.ableton.com/en/about/"
                    className="inline-block"
                  >
                    <div className="flex items-center">
                      <p>About Ableton</p>
                      <FooterArrow />
                    </div>
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.ableton.com/en/jobs/"
                    className="inline-block"
                  >
                    <div className="flex items-center">
                      <p>Jobs</p>
                      <FooterArrow />
                    </div>
                  </a>
                </li>
              </ul>
              <div className="flex pt-3 gap-1">
                <a
                  href="https://www.facebook.com/ableton"
                  className="inline-block"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="w-10 h-10 min-[420px]:w-12 min-[420px]:h-12"
                    width="240"
                    height="240"
                    viewBox="0 0 24 24"
                  >
                    <path d="M12.001 2.002c-5.522 0-9.999 4.477-9.999 9.999 0 4.99 3.656 9.126 8.437 9.879v-6.988h-2.54v-2.891h2.54V9.798c0-2.508 1.493-3.891 3.776-3.891 1.094 0 2.24.195 2.24.195v2.459h-1.264c-1.24 0-1.628.772-1.628 1.563v1.875h2.771l-.443 2.891h-2.328v6.988C18.344 21.129 22 16.992 22 12.001c0-5.522-4.477-9.999-9.999-9.999z"></path>
                  </svg>
                </a>
                <a href="https://twitter.com/ableton" className="inline-block">
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="w-10 h-10 min-[420px]:w-12 min-[420px]:h-12"
                    width="240"
                    height="240"
                    viewBox="0 0 24 24"
                  >
                    <path d="M19.633 7.997c.013.175.013.349.013.523 0 5.325-4.053 11.461-11.46 11.461-2.282 0-4.402-.661-6.186-1.809.324.037.636.05.973.05a8.07 8.07 0 0 0 5.001-1.721 4.036 4.036 0 0 1-3.767-2.793c.249.037.499.062.761.062.361 0 .724-.05 1.061-.137a4.027 4.027 0 0 1-3.23-3.953v-.05c.537.299 1.16.486 1.82.511a4.022 4.022 0 0 1-1.796-3.354c0-.748.199-1.434.548-2.032a11.457 11.457 0 0 0 8.306 4.215c-.062-.3-.1-.611-.1-.923a4.026 4.026 0 0 1 4.028-4.028c1.16 0 2.207.486 2.943 1.272a7.957 7.957 0 0 0 2.556-.973 4.02 4.02 0 0 1-1.771 2.22 8.073 8.073 0 0 0 2.319-.624 8.645 8.645 0 0 1-2.019 2.083z"></path>
                  </svg>
                </a>
                <a
                  href="https://www.youtube.com/ableton"
                  className="inline-block"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="w-10 h-10 min-[420px]:w-12 min-[420px]:h-12"
                    width="240"
                    height="240"
                    viewBox="0 0 24 24"
                  >
                    <path d="M21.593 7.203a2.506 2.506 0 0 0-1.762-1.766C18.265 5.007 12 5 12 5s-6.264-.007-7.831.404a2.56 2.56 0 0 0-1.766 1.778c-.413 1.566-.417 4.814-.417 4.814s-.004 3.264.406 4.814c.23.857.905 1.534 1.763 1.765 1.582.43 7.83.437 7.83.437s6.265.007 7.831-.403a2.515 2.515 0 0 0 1.767-1.763c.414-1.565.417-4.812.417-4.812s.02-3.265-.407-4.831zM9.996 15.005l.005-6 5.207 3.005-5.212 2.995z"></path>
                  </svg>
                </a>
                <a
                  href="https://www.instagram.com/ableton/"
                  className="inline-block"
                >
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    className="w-10 h-10 min-[420px]:w-12 min-[420px]:h-12"
                    width="240"
                    height="240"
                    viewBox="0 0 24 24"
                  >
                    <path d="M11.999 7.377a4.623 4.623 0 1 0 0 9.248 4.623 4.623 0 0 0 0-9.248zm0 7.627a3.004 3.004 0 1 1 0-6.008 3.004 3.004 0 0 1 0 6.008z"></path>
                    <circle cx="16.806" cy="7.207" r="1.078"></circle>
                    <path d="M20.533 6.111A4.605 4.605 0 0 0 17.9 3.479a6.606 6.606 0 0 0-2.186-.42c-.963-.042-1.268-.054-3.71-.054s-2.755 0-3.71.054a6.554 6.554 0 0 0-2.184.42 4.6 4.6 0 0 0-2.633 2.632 6.585 6.585 0 0 0-.419 2.186c-.043.962-.056 1.267-.056 3.71 0 2.442 0 2.753.056 3.71.015.748.156 1.486.419 2.187a4.61 4.61 0 0 0 2.634 2.632 6.584 6.584 0 0 0 2.185.45c.963.042 1.268.055 3.71.055s2.755 0 3.71-.055a6.615 6.615 0 0 0 2.186-.419 4.613 4.613 0 0 0 2.633-2.633c.263-.7.404-1.438.419-2.186.043-.962.056-1.267.056-3.71s0-2.753-.056-3.71a6.581 6.581 0 0 0-.421-2.217zm-1.218 9.532a5.043 5.043 0 0 1-.311 1.688 2.987 2.987 0 0 1-1.712 1.711 4.985 4.985 0 0 1-1.67.311c-.95.044-1.218.055-3.654.055-2.438 0-2.687 0-3.655-.055a4.96 4.96 0 0 1-1.669-.311 2.985 2.985 0 0 1-1.719-1.711 5.08 5.08 0 0 1-.311-1.669c-.043-.95-.053-1.218-.053-3.654 0-2.437 0-2.686.053-3.655a5.038 5.038 0 0 1 .311-1.687c.305-.789.93-1.41 1.719-1.712a5.01 5.01 0 0 1 1.669-.311c.951-.043 1.218-.055 3.655-.055s2.687 0 3.654.055a4.96 4.96 0 0 1 1.67.311 2.991 2.991 0 0 1 1.712 1.712 5.08 5.08 0 0 1 .311 1.669c.043.951.054 1.218.054 3.655 0 2.436 0 2.698-.043 3.654h-.011z"></path>
                  </svg>
                </a>
              </div>
            </div>
            <div className="max-lg:mt-8">
              <header>
                <p className="font-medium">Education</p>
              </header>
              <div>
                <a
                  href="https://www.ableton.com/en/shop/education/"
                  className="inline-block"
                >
                  <div className="flex items-center">
                    <p>Offers for students and teachers</p>
                    <FooterArrow />
                  </div>
                </a>
              </div>
              <div>
                <a
                  href="https://www.ableton.com/en/classroom/"
                  className="inline-block"
                >
                  <div className="flex items-center">
                    <p>Ableton for the Classroom</p>
                    <FooterArrow />
                  </div>
                </a>
              </div>
              <div>
                <a
                  href="https://www.ableton.com/en/colleges-universities/"
                  className="inline-block"
                >
                  <div className="flex items-center">
                    <p>Ableton for Colleges and Universities</p>
                    <FooterArrow />
                  </div>
                </a>
              </div>
            </div>
            <div className="max-lg:my-9 lg:my-7">
              <header>
                <p className="font-medium">Community</p>
              </header>
              <div>
                <a
                  href="https://www.ableton.com/en/community/user-groups/"
                  className="inline-block"
                >
                  <div className="flex items-center">
                    <p>Find Ableton User Groups</p>
                    <FooterArrow />
                  </div>
                </a>
              </div>
              <div>
                <a
                  href="https://www.ableton.com/en/certified-training/"
                  className="inline-block"
                >
                  <div className="flex items-center">
                    <p>Find Certified Training</p>
                    <FooterArrow />
                  </div>
                </a>
              </div>
              <div>
                <a
                  href="https://www.ableton.com/en/education/certification-program/"
                  className="inline-block"
                >
                  <div className="flex items-center">
                    <p>Become a Certified Trainer</p>
                    <FooterArrow />
                  </div>
                </a>
              </div>
            </div>
            <div className="max-lg:mb-8 lg:my-7">
              <header>
                <p className="font-medium">Distributors</p>
              </header>
              <div>
                <a
                  href="https://www.ableton.com/en/distributors/"
                  className="inline-block"
                >
                  <div className="flex items-center">
                    <p>Find Distributors</p>
                    <FooterArrow />
                  </div>
                </a>
              </div>
              <div>
                <a
                  href="https://www.ableton.com/en/try-push-in-store/"
                  className="inline-block"
                >
                  <div className="flex items-center">
                    <p>Try Push in-store</p>
                    <FooterArrow />
                  </div>
                </a>
              </div>
            </div>
          </div>
          <div className="order-3 basis-full lg:flex items-center justify-between text-[0.83rem] font-medium lg:mt-10">
            <div className="max-lg:space-y-1.5 lg:flex gap-4">
              <div>
                <a
                  href="https://www.ableton.com/en/contact-us/"
                  className="inline-block"
                >
                  <p>Contact Us</p>
                </a>
              </div>
              <div>
                <a
                  href="https://www.ableton.com/en/press/"
                  className="inline-block"
                >
                  <p>Press Resources</p>
                </a>
              </div>
              <div>
                <a
                  href="https://www.ableton.com/en/legal/"
                  className="inline-block"
                >
                  <p>Legal Info</p>
                </a>
              </div>
              <div>
                <a
                  href="https://www.ableton.com/en/privacy/"
                  className="inline-block"
                >
                  <p>Privacy Policy</p>
                </a>
              </div>
              <div>
                <a
                  href="https://www.ableton.com/en/cookie-settings/"
                  className="inline-block"
                >
                  <p>Cookie Settings</p>
                </a>
              </div>
              <div>
                <a
                  href="https://www.ableton.com/en/imprint/"
                  className="inline-block"
                >
                  <p>Imprint</p>
                </a>
              </div>
            </div>
            <div className="flex items-center gap-4 max-lg:pt-8 lg:flex-row-reverse">
              <div>
                <svg
                  className="w-[3rem] h-auto object-contain"
                  xmlns="http://www.w3.org/2000/svg"
                  width="45"
                  height="21"
                  viewBox="0 0 45 21"
                >
                  <path d="M0 0h3v21H0zM6 0h3v21H6zM12 0h3v21h-3zM18 0h3v21h-3zM24 18h21v3H24zM24 12h21v3H24zM24 6h21v3H24zM24 0h21v3H24z"></path>
                </svg>
              </div>
              <div>
                <p>Made in Berlin</p>
              </div>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}

export default Footer;
